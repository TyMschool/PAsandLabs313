/*
    ============================================================
    Introduction to fork(), exec(), pipe(), and wait() for lab2
    ============================================================
...
int main() {
    ...
    // - Child gets 0
    pid_t pid = fork(); //tells you the STATUS of the fork. -1 failed, 0 is the child, else parent
    ...
    if (pid == 0) {
        ...
        cout << "Hello from the CHILD process! (pid=" << getpid() << ")\n"; //this gets you the actual pid of the child PROCESS 
        return 0; // child exits here
    } else {
        // This branch runs in the PARENT process
        cout << "Hello from the PARENT process! (pid=" << getpid() //same thing, gets the parent PID
             << "), child=" << pid << "\n";
